...
[t,X] = ode23tb(@(t,X) bubble(t,X,cav_type,params),[0 tspan_star], X0, options);

% Extract the solution
R = X(:,1); % Bubble wall radius history
U = X(:,2); % Bubble wall velocity history
P = X(:,3); % Internal bubble pressure history
S = X(:,4); % Stress integral history
Theta = X(:,5:(NT+4)); % Variable relating to internal temp (theta)
k = X(:,(NT+5):(2*NT+4)); % Vapor concentration in the bubble
T = (A_star - 1 + sqrt(1+2.*A_star.*Theta))./A_star; % Dimensionless temp in bubble
